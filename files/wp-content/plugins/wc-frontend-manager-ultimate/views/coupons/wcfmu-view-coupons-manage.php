...
$klippkort_val = get_post_meta($coupon_id, 'klippkort', true);
echo "<hr>";
print_r(["klippkort_val" => $klippkort_val, "is_vendor" => wcfm_is_vendor()]);
echo "<hr>";
...
$_products_objs = $WCFM->wcfm_vendor_support->wcfm_get_products_by_vendor( apply_filters( 'wcfm_current_vendor_id', get_current_user_id() ), 'publish' );

$_product_ids = [];
if( !empty($_products_objs) ) {
    foreach( $_products_objs as $products_obj )  {
        $_product_ids[] = $products_obj->ID;
    }
}
...
foreach($entries as $key => $value) {
    if (!wcfm_is_vendor() || in_array($value->ID, $_product_ids))
        $klippkort[$value->ID] = $value->post_title;
}
                ...
                $WCFM->wcfm_fields->wcfm_generate_form_field(
                    apply_filters(
                        ...
                        [
                            ...
                            'klippkort' => [
                                'label' => __('Koppla till Klippkort', 'wc-frontend-manager-ultimate'),
                                 'type' => 'radio', 
                                 'options' => $klippkort,
                                 /*'type' => 'text',*/
                                 'value' => $klippkort_val,
                ...
                <p>
                    ...
                    $WCFM->wcfm_fields->wcfm_generate_form_field(
                        apply_filters(
                            ...
                            [
                                'usage_limit'            => [
                                    'label'       => __('Usage limit per couponnn', 'wc-frontend-manager-ultimate'),
                                    'type'        => 'number',
