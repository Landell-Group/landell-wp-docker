...
class WC_API_Coupons extends WC_API_Resource {
	...
	public function get_coupon( $id, $fields = null ) {
		try {
			...
			$coupon_data = array(
				...
				'klippkort'         		   => $coupon->get_prop( 'klippkort', 'view' ) ? $coupon->get_prop( 'klippkort', 'view' ) : null, /** LANDELL 2.0 **/
	...
	public function create_coupon( $data ) {
		...
		try {
			...
			update_post_meta( $id, 'klippkort', wc_clean( $coupon_data['klippkort'] ) ); /** LANDELL 2.0 **/
	...
	public function edit_coupon( $id, $data ) {
		...
		try {
			...
			if ( isset( $data['klippkort'] ) ) { /** LANDELL 2.0 **/
				update_post_meta( $id, 'klippkort', wc_clean( $data['klippkort'] ) );
			}
